        ...
        <div>
            ...
            <form:form action="/WebApplication2/app/bus/save" method="post">
                <table>
                    ...
                    <tr>
                        <td>N° matricule :</td>
                        <td><input name="noMatricule" value="${bus.getNoMatricule()}"/></td>
                    </tr
                    <tr>
                        <td>Nombre de passagers max :</td>
                        <td><input name="nbPassagerMax" value="${bus.getNbPassagerMax()}"/></td>
                    </tr>
                    <tr>
                        <td>Transport pour handicap :</td>
                        <td>
                            <input type="checkbox" name="transportHandicap" 
                                   <c:if test="${bus.getHandicap()}">checked</c:if>
                        </td>
                    </tr>
                    <tr>
                        <td>Model :</td>
                            ...
                            <select name="modelId">
                                <c:forEach items="${models}" var="mo">
                                    <option value="${mo.getId()}"
                                             <c:if test="${bus.getModel().getId() == mo.getId()}">selected</c:if>
                            ...
                            <select name="categorieId">
                                <c:forEach items="${categories}" var="ca">
                                    <option value="${ca.getId()}"
                                             <c:if test="${bus.getCategorie().getId() == ca.getId()}">selected</c:if>
                                             >${ca.getNom()}
                                    </option>
                                </c:forEach>
                            </select>
